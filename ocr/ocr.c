...
int main()
{
    ...
    tick = clock();
    // test 100 images 3 layers 
    float **images = read_images("test-images.dat", &n_images, &n_rows, &n_cols); // real image pixel
    ...
    {
        memcpy((void *) nn.neurons, (void *) images[idx], nn.n_neurons[0]*sizeof(float));
        // excute neuronet_eval is software excute
        class_id = neuronet_eval_hardware(&nn, images[idx]);
        // excute neuronet_eval is hardware excute
